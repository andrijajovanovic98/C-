...
{
	if (theForm.getGradeToSign() < this->grade && theForm.getGradeToExecute() < this->grade)
	{
		std::cout << this->name << " couldnt sign " << theForm.getFormName() << " because of to low grade" << std::endl;
		throw GradeTooLowException();
	}
	else {
	theForm.beSigned(*this);
	std::cout << this->name << " signed " << theForm.getFormName() << std::endl;;
	}
}
...
void Bureaucrat::executeForm(Form const & Form)
{
	...
	/* catch(const std::exception &e) {
		std::cout << this->name << " couldnt execute " << Form.getFormName() << " because " <<  e.what() << std::endl;
	} */
}
